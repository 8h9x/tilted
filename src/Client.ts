...
import XMPP from "./xmpp/XMPP.ts";
import Friend from "./structures/friend/Friend.ts";
import UserNotFoundError from "./exceptions/UserNotFoundError.ts";
import StatsPrivacyError from "./exceptions/StatsPrivacyError.ts";
import CreatorCode from "./structures/CreatorCode.ts";
import CreatorCodeNotFoundError from "./exceptions/CreatorCodeNotFoundError.ts";
import FriendNotFoundError from "./exceptions/FriendNotFoundError.ts";
import IncomingPendingFriend from "./structures/friend/IncomingPendingFriend.ts";
import OutgoingPendingFriend from "./structures/friend/OutgoingPendingFriend.ts";
import BlockedUser from "./structures/user/BlockedUser.ts";
import ClientParty from "./structures/party/ClientParty.ts";
import Party from "./structures/party/Party.ts";
import PartyNotFoundError from "./exceptions/PartyNotFoundError.ts";
import PartyPermissionError from "./exceptions/PartyPermissionError.ts";
import SentPartyJoinRequest from "./structures/party/SentPartyJoinRequest.ts";
import RadioStation from "./structures/RadioStation.ts";
import CreativeIslandNotFoundError from "./exceptions/CreativeIslandNotFoundError.ts";
import Stats from "./structures/Stats.ts";
import EventTimeoutError from "./exceptions/EventTimeoutError.ts";
import FortniteServerStatus from "./structures/FortniteServerStatus.ts";
import EpicgamesServerStatus from "./structures/EpicgamesServerStatus.ts";
...
import FriendManager from "./managers/FriendManager.ts";
import STWManager from "./managers/STWManager.ts";
import STOMP from "./stomp/STOMP.ts";
import ChatManager from "./managers/ChatManager.ts";
import type { PresenceShow } from "stanza/Constants.ts";
import type {
  BlurlStreamData,
...
import type { SentPartyInvitation } from "./mod.ts";
...
class Client extends EventTarget {
  ...
  /**
   * Listeners set by {@link Client#addEventListener} that have not been removed
   */
  // eslint-disable-next-line no-undef
    ...
    this.config = {
      ...
      auth: {
        authorizationCode: async () =>
          consoleQuestion("Please enter an authorization code: "),
        checkEULA: true,
  ...
  override addEventListener<K extends keyof ClientEventMap>(
    type: K,
    callback: (e: CustomEvent<ClientEventMap[K]>) => void,
  ...
  createEvent<K extends keyof ClientEventMap>(
    type: K,
    detail: ClientEventMap[K],
  ): CustomEvent<ClientEventMap[K]> {
    return new CustomEvent(type, { detail });
  }

  override dispatchEvent<K extends keyof ClientEventMap>(
    event: CustomEvent<ClientEventMap[K]>,
  ): boolean;
  ...
  override removeEventListener(
    ...
    else this.listenerCounts.set(type, count);
  }

  listenerCount<K extends keyof ClientEventMap>(eventName: K): number;
   ...
   * @private
   */
  public setClientParty(party: Party): void {
  ...
  public async waitUntilReady(timeout = 10000): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        ...
        reject(new Error(`Ready timeout exceeded after ${timeout}ms`));
      }, timeout);
  ...
  private destroy(): void {
    ...
    const launcherSession = this.auth.sessions.get(
      AuthSessionStoreKey.Launcher,
    );
    if (launcherSession?.refreshTimeout) {
      this.clearTimeout(launcherSession.refreshTimeout);
    }
   ...
   */
  private initCacheSweeping(): void {
    ...
    if (
      ...
      userCacheSettings.sweepInterval > 0 &&
      userCacheSettings.maxLifetime > 0 &&
      userCacheSettings.maxLifetime !== Infinity
    ) {
      this.setInterval(
  ...
  public sweepPresences(
    maxLifetime: number | undefined = this.config.cacheSettings.presences
      ?.maxLifetime,
  ): number {
   ...
   */
  public sweepUsers(
    maxLifetime: number | undefined = this.config.cacheSettings.users
      ?.maxLifetime,
  ): number {
    if (typeof maxLifetime !== "number") {
      throw new TypeError("maxLifetime must be typeof number");
   ...
   */
  public waitForEvent<K extends keyof ClientEventMap>(
    event: K,
    timeout = 5000,
    // eslint-disable-next-line no-unused-vars
    filter?: (detail: ClientEventMap[K]) => boolean,
  ): Promise<ClientEventMap[K]> {
    return new Promise((res, rej) => {
      let rejectionTimeout: ReturnType<typeof setTimeout>;

      const handler = (e: Event) => {
        const detail = (e as CustomEvent<ClientEventMap[K]>).detail;
   ...
   */
  // eslint-disable-next-line no-undef
  public clearInterval(interval: ReturnType<typeof setTimeout>): void {
    clearInterval(interval);
    this.intervals.delete(interval);
  }

  public static consoleQuestion(question: string): Promise<string> {
  ...
  public debug(
    message: string,
    type: "regular" | "http" | "xmpp" | "stomp" = "regular",
  ): void {
    switch (type) {
      case "regular":
  ...
  public setStatus(
    ...
    const rawOnlineType =
      (onlineType || this.config.defaultOnlineType) === "online"
        ? undefined
   ...
   * @throws {EpicgamesAPIError}
   */
  public async invite(friend: string): Promise<SentPartyInvitation> {
  ...
  public async createParty(config?: PartyConfig): Promise<void> {
    ...
    try {
      ...
      if (
        ...
        e.code === "errors.com.epicgames.social.party.user_has_party"
      ) {
        await this.leaveParty(false);
   ...
   * @throws {EpicgamesAPIError}
   */
  public async leaveParty(createNew = true): Promise<void> {
   ...
   * @throws {EpicgamesAPIError}
   */
  public async sendRequestToJoin(
    friend: string,
  ): Promise<SentPartyJoinRequest> {
    const resolvedFriend = this.friend.list.find(
    ...
    try {
      intention = await this.http.epicgamesRequest(
        {
          method: "POST",
          url: `${Endpoints.BR_PARTY}/members/${resolvedFriend.id}/intentions/${
            this.user.self!.id
          }`,
          headers: {
            "Content-Type": "application/json",
      ...
      if (
        e instanceof EpicgamesAPIError &&
        e.code === "errors.com.epicgames.social.party.user_has_no_party"
      ) {
   ...
   * @throws {EpicgamesAPIError}
   */
  public async getClientParty(): Promise<ClientParty | undefined> {
   ...
   * @throws {EpicgamesAPIError}
   */
  public async getStorefronts(language: Language = "en"): Promise<any> {
  ...
  public async getCreatorCode(code: string): Promise<CreatorCode> {
    ...
    try {
      ...
      if (
        e instanceof EpicgamesAPIError &&
        e.code === "errors.com.epicgames.ecommerce.affiliate.not_found"
      ) {
  ...
  public async getRadioStations(): Promise<RadioStation[]> {
    ...
    const radioStations =
      fortniteContent.radioStations.radioStationList.stations;

    return radioStations.map((s: unknown) => new RadioStation(this, s));
  }
   ...
   * @throws {EpicgamesAPIError}
   */
  public async getBREventFlags(language: Language = "en"): Promise<unknown> {
  ...
  public async getCreativeIsland(code: string): Promise<CreativeIslandData> {
    ...
    try {
      ...
      if (
        e instanceof EpicgamesAPIError &&
        e.code === "errors.com.epicgames.links.no_active_version"
      ) {
  ...
  public async getCreativeDiscoveryPanels(
    ...
    const creativeDiscovery = await this.http.epicgamesRequest(
      {
        method: "POST",
        url: `${Endpoints.CREATIVE_DISCOVERY}/${
          this.user.self!.id
        }?appId=Fortnite`,
        headers: {
          "Content-Type": "application/json",
