...
import FortniteAuthSession from "./FortniteAuthSession.ts";
import LauncherAuthSession from "./LauncherAuthSession.ts";
import AuthClients from "../resources/AuthClients.ts";
import { resolveAuthObject, resolveAuthString } from "../util/Util.ts";
import EpicgamesAPIError from "../exceptions/EpicgamesAPIError.ts";
import FortniteClientCredentialsAuthSession from "./FortniteClientCredentialsAuthSession.ts";
import EOSAuthSession from "./EOSAuthSession.ts";
import type {
  AuthClient,
  AuthSessionStore,
  AuthStringResolveable,
  DeviceAuthResolveable,
  DeviceAuthWithSnakeCaseSupport,
} from "../resources/structs.ts";
import type Client from "../Client.ts";

type AuthSessionStoreType = AuthSessionStore<
  AuthSessionStoreKey,
  | FortniteAuthSession
  | LauncherAuthSession
...
class Auth extends Base {
  ...
  public async authenticate() {
    ...
    if (authCreds.launcherRefreshToken) {
      await this.launcherRefreshTokenAuthenticate(
        authCreds.launcherRefreshToken,
        authClient,
      );
      ...
      await this.authorizationCodeAuthenticate(
        authCreds.authorizationCode,
        authClient,
      );
    } else {
      throw new Error(
        "No valid auth method found! Please provide one in the client config",
      );
    }

    if (
      (!authCreds.launcherRefreshToken &&
        this.client.listenerCount("refreshtoken:created") > 0) ||
      (authCreds.createLauncherSession &&
        !this.sessions.has(AuthSessionStoreKey.Launcher))
    ) {
      const exchangeCode = await this.sessions
        .get(AuthSessionStoreKey.Fortnite)!
        .createExchangeCode();
    ...
    if (
      !authCreds.deviceAuth &&
      this.client.listenerCount("deviceauth:created") > 0
    ) {
      ...
      this.client.dispatchEvent(
        this.client.createEvent("deviceauth:created", { deviceAuth }),
      );
    }

    const fortniteClientCredsSession =
      await FortniteClientCredentialsAuthSession.create(
        this.client,
  ...
  public async revokeAllTokens() {
    await Promise.all(
      [
        ...this.sessions
          .filter((_s, k) => k !== AuthSessionStoreKey.Launcher)
          .values(),
      ].map((s) => s.revoke()),
    );
  }
  ...
  private async acceptEULA() {
    const EULAdata = await this.client.http.epicgamesRequest(
      {
        url: `${Endpoints.INIT_EULA}/account/${
    ...
    try {
      await this.client.http.epicgamesRequest(
        {
          method: "POST",
          url: `${Endpoints.INIT_GRANTACCESS}/${
            this.sessions.get(AuthSessionStoreKey.Fortnite)!.accountId
          }`,
        },
        AuthSessionStoreKey.Fortnite,
      ...
      if (
        ...
        e.message ===
          "Client requested access grant but already has the requested access entitlement"
      ) {
  ...
  private async createDeviceAuth() {
    return await this.client.http.epicgamesRequest(
      {
        method: "POST",
        url: `${Endpoints.OAUTH_DEVICE_AUTH}/${
          this.sessions.get(AuthSessionStoreKey.Fortnite)?.accountId
        }/deviceAuth`,
      },
      AuthSessionStoreKey.Fortnite,
   ...
   */
  private async deviceAuthAuthenticate(
   ...
   */
  private async exchangeCodeAuthenticate(
    exchangeCodeResolvable: AuthStringResolveable,
    authClient: AuthClient,
  ) {
   ...
   */
  private async authorizationCodeAuthenticate(
    authorizationCodeResolvable: AuthStringResolveable,
    authClient: AuthClient,
  ) {
    ...
    switch (typeof authorizationCodeResolvable) {
      ...
      case "string":
        if (authorizationCodeResolvable.length === 32) {
          ...
          authorizationCode = (
            await fs.readFile(authorizationCodeResolvable)
          ).toString();
        }
        break;
      default:
        throw new TypeError(
          `The type "${typeof authorizationCodeResolvable}" does not resolve to a valid auth string`,
        );
    }
   ...
   */
  private async refreshTokenAuthenticate(
    refreshTokenResolvable: AuthStringResolveable,
    authClient: AuthClient,
  ) {
   ...
   * @param refreshTokenResolvable A resolvable refresh token
   */
  private async launcherRefreshTokenAuthenticate(
    refreshTokenResolvable: AuthStringResolveable,
    authClient: AuthClient,
  ) {
    const refreshToken = await resolveAuthString(refreshTokenResolvable);
