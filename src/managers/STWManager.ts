...
import STWProfile from "../structures/stw/STWProfile.ts";
import STWNewsMessage from "../structures/stw/STWNewsMessage.ts";
import type { STWWorldInfoData } from "../resources/structs.ts";
...
class STWManager extends Base {
   ...
   */
  public async getProfile(user: string): Promise<STWProfile> {
    ...
    try {
      ...
      if (
        ...
        e.code === "errors.com.epicgames.modules.profiles.profile_not_found"
      ) {
    ...
    return new STWProfile(
      this.client,
      queryProfileResponse.profileChanges[0].profile,
      resolvedUser,
    );
  }
  ...
  public async getNews(
    ...
    return newsResponse.news.messages.map(
      (m: any) => new STWNewsMessage(this.client, m),
    );
  }
