...
import MatchNotFoundError from "../exceptions/MatchNotFoundError.ts";
import EventTokens from "../structures/EventTokens.ts";
import Tournament from "../structures/Tournament.ts";
...
class TournamentManager extends Base {
   ...
   */
  private async downloadReplayCDNFile(url: string) {
    ...
    const statsResponses = await Promise.all(
      userChunks.map((c) =>
        this.client.http.epicgamesRequest(
          {
            method: "GET",
            url: `${Endpoints.BR_TOURNAMENT_TOKENS}?teamAccountIds=${
              c.join(
                ",",
              )
            }`,
  ...
  public async get(
      ...
      this.client.http.epicgamesRequest(
        {
          method: "GET",
          url:
            `${Endpoints.BR_TOURNAMENTS_DOWNLOAD}/${
    ...
    tournaments.events.forEach((t: TournamentData) => {
      let tournamentDisplayData = tournamentsInfo.tournament_info?.tournaments
        ?.find(
          (td: TournamentDisplayData) =>
            td.tournament_display_id === t.displayDataId,
        );

      if (!tournamentDisplayData) {
        tournamentDisplayData = (Object.values(tournamentsInfo) as any[]).find(
          (tdr: any) =>
            tdr.tournament_info?.tournament_display_id === t.displayDataId,
        )?.tournament_info;
      }
      ...
      const windowsResolvedData: Map<string, TournamentWindowResolvedData[]> =
        new Map();

      t.eventWindows.forEach((w) => {
        const template = tournaments.templates.find(
          (tt: TournamentWindowTemplateData) =>
            tt.eventTemplateId === w.eventTemplateId,
        );
        if (template) {
        ...
        const resolvedDataForWindow: TournamentWindowResolvedData[] = w
          .scoreLocations.map((scoreLocation) => {
  ...
  public async getData(
    ...
    const tournaments: TournamentsResponse = await this.client.http
      .epicgamesRequest(
        {
          method: "GET",
          url: `${Endpoints.BR_TOURNAMENTS}/${
            this.client.user.self!.id
  ...
  public async getSessionMetadata(
    ...
    try {
      replayMetadataResponse = await this.downloadReplayCDNFile(
      ...
      if (
        ...
        e.data.includes("<Message>The specified key does not exist.</Message>")
      ) {
    ...
    const window = await this.client.http.epicgamesRequest(
      {
        ...
        url: `${Endpoints.BR_TOURNAMENT_WINDOW}/${eventId}/${eventWindowId}/` +
          `${
            this.client.user.self!.id
          }?page=${page}&rank=0&teamAccountIds=&appId=Fortnite&showLiveSessions=${showLiveSessions}`,
      },
      AuthSessionStoreKey.Fortnite,
    );
