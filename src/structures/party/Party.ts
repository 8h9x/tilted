...
import { PartyPrivacy } from "../../enums/Enums.ts";
import Base from "../../Base.ts";
import PartyAlreadyJoinedError from "../../exceptions/PartyAlreadyJoinedError.ts";
import { makeCamelCase, makeSnakeCase } from "../../util/Util.ts";
import ClientPartyMember from "./ClientPartyMember.ts";
...
class Party extends Base {
  ...
  constructor(client: Client, data: PartyData) {
    ...
    this.config.privacy = this.config.joinability === "OPEN"
      ? PartyPrivacy.PUBLIC
    ...
    this.members = new Collection(
      data.members.map((m) => {
        if (m.account_id === this.client.user.self!.id) {
          return [m.account_id, new ClientPartyMember(this, m)];
        }
   ...
   * The party's leader
   */
  public get leader(): PartyMember | ClientPartyMember | undefined {
   ...
   */
  public get playlist(): Island | undefined {
  ...
  public async join(skipRefresh = false) {
    ...
    try {
      await this.client.http.epicgamesRequest(
        {
          method: "POST",
          url: `${Endpoints.BR_PARTY}/parties/${this.id}/members/${
  ...
  public updateData(data: PartyUpdateData) {
    ...
    this.meta.remove(
      (data.party_state_removed as (keyof PartySchema & string)[]) ?? [],
    );
    ...
    let privacy = this.meta.get("Default:PrivacySettings_j") as any;
    privacy = Object.values(PartyPrivacy).find(
      (val) =>
        val.partyType === privacy.PrivacySettings.partyType &&
        val.inviteRestriction ===
          privacy.PrivacySettings.partyInviteRestriction &&
        val.onlyLeaderFriendsCanJoin ===
          privacy.PrivacySettings.bOnlyLeaderFriendsCanJoin,
    );
  ...
  public async updateMemberBasicInfo() {
    const users = await this.client.user.fetchMultiple(
      this.members.map((m: PartyMember) => m.id),
    );
