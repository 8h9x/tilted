...
import { STWLeadSynergy } from "../enums/Enums.ts";
...
export const consoleQuestion = (question: string) =>
...
export const getRandomDefaultCharacter = () =>
  defaultCharacters[Math.floor(Math.random() * defaultCharacters.length)];

export const createPartyInvitation = (
  clientUserId: string,
  pingerId: string,
  data: any,
) => {
...
export const parseBlurlStream = (stream: Uint8Array) =>
  new Promise<any>((res) => {
...
const buildReplayMeta = (replay: BinaryWriter, replayData: ReplayData) => {
  replay
    ...
    .writeUInt64(
      BigInt(new Date(replayData.Timestamp).getTime()) * BigInt("10000") +
...
const buildChunks = (replay: BinaryWriter, replayData: ReplayData) => {
  ...
  for (const chunk of chunks) {
    ...
    replay.goto(chunkSizeOffset).writeInt32(chunkSize).goto(savedOffset);
  }
};
...
export const buildReplay = (replayData: ReplayData, addStats: boolean) => {
  ...
  const finalReplayByteLength = 562 + // meta
    8 +
    replayData.Header.length + // header
    (replayData.DataChunks?.map((c) => 8 + 16 + c.data.length).reduce(
      (acc, cur) => acc + cur,
    ) || 0) + // datachunks
    (replayData.Events?.map(
      (e) =>
        8 +
        12 +
        e.Id.length +
        ...
        e.Group.length +
        5 +
        (e.Metadata ? e.Metadata.length + 5 : 5) +
        e.data.length,
    ).reduce((acc, cur) => acc + cur) || 0) + // events
    (replayData.Checkpoints?.map(
      (c) =>
        8 +
        12 +
        c.Id.length +
        ...
        c.Group.length +
        5 +
        (c.Metadata ? c.Metadata.length + 5 : 5) +
        c.data.length,
    ).reduce((acc, cur) => acc + cur) || 0); // checkpoints

  const replay = new BinaryWriter(new Uint8Array(finalReplayByteLength));
...
export const parseSTWSurvivorTemplateId = (templateId: string) => {
  ...
  const rarity = (
    type === "manager" ? fields.shift() : fields.pop()
  ) as STWItemRarity;
...
export const calcSTWSurvivorPowerLevel = (
...
export const calcSTWSurvivorLeadBonus = (
  managerSynergy: string,
  squadName: keyof STWSurvivorSquads,
  powerLevel: number,
) => {
...
export const calcSTWNonSurvivorPowerLevel = (
  rarity: STWItemRarity,
  level: number,
  tier: STWItemTier,
...
export function parseSTWSchematicTemplateId(templateId: string): {
  ...
  const tier = parseInt(fields.pop()!.slice(1), 10) as STWItemTier;
  const evoType = type !== "trap"
    ? (fields.pop() as STWSchematicEvoType)
    : undefined;
  const rarity = fields.pop() as STWItemRarity;
  const name = fields[0] ? fields.join("_") : undefined;
...
export const parseStatKey = (
  key: string,
  value: number,
): [keyof StatsPlaylistTypeData, number | Date] => {
  switch (key) {
    case "lastmodified":
...
export const resolveAuthString = async (str: AuthStringResolveable) => {
  switch (typeof str) {
    ...
    default:
      throw new TypeError(
        `The type "${typeof str}" does not resolve to a valid auth string`,
      );
  }
};
...
export const resolveAuthObject = async (obj: DeviceAuthResolveable) => {
  switch (typeof obj) {
    ...
    default:
      throw new TypeError(
        `The type "${typeof obj}" does not resolve to a valid auth object`,
      );
  }
};
